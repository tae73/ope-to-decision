...
같은 로그·같은 importance weight 로 지표 벡터(CTR·CVR·REV)를 한 번에 평가할 수 있다 — estimator 에서
바뀌는 것은 reward 벡터뿐이다. 그러나 **신뢰는 지표마다 같지 않다**: funnel 이 깊어질수록(클릭 → 전환 →
매출) 이벤트가 희소해지고, REV 는 액션별 price 의 heavy tail 까지 얹혀, 같은 n 에서 "2.8·SE(Δ̂)" 기준의
최소 판별 가능 효과가 지표 깊이를 따라 가팔라진다 — 신뢰도의 사다리다(축 15 figure/CSV 참조).

- **실무 규칙: 지표별 판별 한계를 지표별로 확인하라.** CTR 에서 판별 가능한 로그가 REV 에선 판별
  불능일 수 있다 — "CTR 로 게이트를 통과했으니 매출도 읽을 수 있다"는 추론은 무근거다(§8.3).
- **CVR 은 세션 기준이다**(= 노출당 click·conv — 업계의 click-조건부 CVR 과 다름, GLOSSARY §7).
  ratio-of-estimates(분모도 추정치인 비율) 함정을 회피하기 위한 정의 선택이며 figure 캡션에 고정 병기한다.
- **불확실성 표기**: 합성 MC 축이므로 seed-ensemble band 를 쓰되, heavy tail 에서 SE 는 과소평가될 수
  있어 **p90 를 병기**한다. 0-이벤트 seed 의 퇴화는 이벤트 수 컬럼으로 정직 기록한다(축 12 계보).

### 8.2 지표별 guardrail 게이트 (축 16) — 비교형 원칙의 벡터 확장

§4 의 비교형 우선 원칙은 지표 벡터로 그대로 확장된다: **deploy iff Δ̂_CTR > 0 ∧ Δ̂_REV ≥ −g ∧
HHI ≤ h** — 각 지표를 같은 로그 위에서 같은 estimator 의 Δ-OPE 로 추정한다(절대형 게이트와의 대비는
축 16 figure/CSV 참조). guardrail 한계 g·h 는 **시연값(무교정)** 으로 §2 임계값과 같은 "제안" 지위를
계승한다. 광고주 층은 성격이 갈린다: 노출 재분배·HHI 는 π 가 기지이므로 **OPE 가 아니라 정확 계산**
(결정적 arm — 오류율 0)인 반면, subgroup 매출 OPE 는 희소해질 수 있어 최소 이벤트 임계(사전 등록)
미달 시 "not estimable" 을 정직 반환한다(`src/ope/business.py` — 발화 여부는 축 16 CSV 기록).

**지표 간 오차 군집 경고.** 중첩 지표는 같은 w_i 를 공유한다 — 극단 weight 를 뽑은 로그(seed)는 세 Δ̂ 를
**동시에** 오염시키고, 결합 게이트의 오류는 지표별 오류의 독립 곱이 아니라 **군집으로** 발생한다.
지표별 false-go 율의 곱으로 결합 오류율을 낙관 추정하지 말 것(seed 단위 기록 — 축 16 CSV). 부속 한계:
`hyperparams_from_weights`(τ=p95(w) 등)는 weight 분위만 보고 **reward 척도를 무시**한다 — clipping/switch
는 price tail 이 만드는 REV 극단 기여를 잡지 못한다.

### 8.3 진단은 지표에 불변 — 게이트 `trust` 의 범위

진단 3종(ESS·max weight·support proxy)은 weight 만 보고 reward 를 보지 않는다 — 따라서 지표 벡터
전체에 대해 **단 하나의 판정만** 낸다. 같은 로그가 CTR 은 판별 가능·REV 는 판별 불능이어도 게이트는 그
차이를 원리적으로 구분하지 못한다(축 08/09 계보의 확장 — 축 15 figure 에 전시). **게이트 `trust` 는
weight 분산 위험의 예보이지, 깊은 지표(REV)의 판별력 보증이 아니다** — trust 판정 후에도 결정에 쓰려는
지표의 판별 한계(§8.1)를 별도로 확인하라.

### 8.4 리텐션 경계 선언 (범위 밖)

세션 간·장기 지표 — retention·LTV·습관화 — 는 **single-step logged bandit OPE 로 식별 불가**다:
본 모형에는 세션 간 상태 전이가 존재하지 않는다. 이 지표들의 OPE 는 RL OPE(FQE·DICE 등) 소관이며 본
레포 범위 밖이다(CLAUDE.md §1 비범위). **세션 내 proxy 도 채택하지 않는다** — 세션 간 상태가 모형에
없으므로 proxy-리텐션 상관이 모형상 정의 불가하고, "proxy 로 리텐션을 봤다"는 서사는 자기기만 위험이
있다. funnel 사다리(축 15)는 세션 내 CTR·CVR·REV 에서 끝나며, 리텐션 단은 의도적으로 없다.
